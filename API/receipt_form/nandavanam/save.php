<?php
    ...
    $status_base_member_id = ""; $funeral_from = ""; $funeral_to = ""; $amount = "";
    ...
    if(isset($json_obj->edit_id)) {
        ...
        if(!empty($json_obj->non_member_name)) {
            $funeral_from = $json_obj->non_member_name;
            $funeral_from = trim($funeral_from);
        }
        if(!empty($funeral_from)) {
            $funeral_from_error = $valid->common_validation($funeral_from, "non_member_name", "1");
        }
        if(empty($member_id_error) && empty($funeral_from_error)) {
            if(empty($member_id) && empty($funeral_from)) {
                $funeral_from_error = "Please select member or enter the non-member name";
            }
            else if(!empty($member_id) && !empty($funeral_from)) {
                $funeral_from_error = "Please select member or enter the non-member name";
            }
        }
        if(!empty($funeral_from_error)) {
            if(empty($valid_receipt)) {
                $valid_receipt = $funeral_from_error;
            }
        }

        $funeral_to = $json_obj->funeral_to;
        $funeral_to = trim($funeral_to);
        $funeral_to_error = $valid->common_validation($funeral_to, "funeral to", "1");
        if(!empty($funeral_to_error)) {
            if(empty($valid_receipt)) {
                $valid_receipt = $funeral_to_error;
            }
        }
        ...
        if(empty($valid_receipt)) {
            ...
            if(empty($member_id)) { $member_id = $GLOBALS['null_value']; }
            if(empty($status_base_member_id)) { $status_base_member_id = $GLOBALS['null_value']; }
            if(!empty($funeral_from) && empty($member_name)) {
                $member_name = $funeral_from;
            }

            if(empty($edit_id)) {
                $null_value = $GLOBALS['null_value'];
                $columns = array('created_date_time', 'creator', 'creator_name', 'receipt_id', 'receipt_type_id', 'receipt_number', 'receipt_date', 'member_id', 'member_name', 'status_base_member_id', 'funeral_to', 'amount', 'deleted');
                $values = array("'".$created_date_time."'", "'".$creator."'", "'".$creator_name."'", "'".$null_value."'", "'".$receipt_type_id."'", "'".$null_value."'", "'".$receipt_date."'", "'".$member_id."'", "'".$member_name."'", "'".$status_base_member_id."'", "'".$funeral_to."'", "'".$amount."'", "'0'");
